...
> [!WARNING]
> Do not select **Exit and continue sleeping**. The standalone projector cannot
> return to the old browser wrapper, so that option can load forever. Close the
> projector and reopen the URL instead. The Pokemon remains asleep and the same
> Dream World session continues.

If Dream Catalogue leaves the room blurred, close and reopen the projector.
The catalogue is not implemented yet.
